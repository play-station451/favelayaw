...
public class SpeedMine extends Module {
    ...
    public final NumberSetting range = num("Range", "Maximum mining distance", 2.0, 7.0, 4.5, 0.1);
    public final NumberSetting speed = num("Speed", "Mining progress multiplier", 0.7, 1.0, 1.0, 0.01);
    public final EnumSetting<Swap> swap = enm("Swap", "Tool swapping mode", Swap.NORMAL);
    public final EnumSetting<Rotate> rotate = enm("Rotate", "Server rotation mode", Rotate.NORMAL);
    ...
    public SpeedMine() {
        super("SpeedMine", "Increases mining speed.", Category.MISC);
    }

    @Override
    public void onEnable() {
        startHandler = Events.on(StartBreakingBlockEvent.class, Priority.HIGH, this::onStartBreaking);
        packetHandler = Events.on(PacketEvent.Receive.class, Priority.HIGH, this::onPacketReceive);
    }
    ...
    public void onUpdate(net.favela.yaw.impl.event.events.UpdateEvent event) {
        if (MC.level == null || MC.player == null || currentTask == null) return;
    ...
    public void onStartBreaking(StartBreakingBlockEvent event) {
        ...
        currentTask = new BlockBreakingTask(pos, event.getDirection(), speed.getFloat());
        lastInstantBreak = 0L;
        lastServerBreak = 0L;
    ...
    @Override
    public void onRenderBlockOutline(net.favela.yaw.impl.event.events.RenderBlockOutlineEvent event) {
        if (currentTask != null && currentTask.getBlockPos() != null) {
            event.cancel();
        }
    }
    ...
    private void handleMiningTick(BlockBreakingTask task) {
        ...
        float delta = calculateBlockDamage(task.getStartState(), pos);
        if (task.incrementProgress(delta) >= task.targetSpeed || task.instantRemine) {
            finishMining(task);
        }
    }

    private void handleDoubleMine(BlockBreakingTask task) {
        if (!doubleMine.get() || MC.player == null || MC.level == null) return;
        ...
        float delta = calculateBlockDamage(task.getStartState(), task.getBlockPos());
    ...
    private void startMining(BlockBreakingTask task) {
        ...
        if (grim.get()) {
            sendAction(ServerboundPlayerActionPacket.Action.STOP_DESTROY_BLOCK, task);
    ...
    private void abortMining(BlockBreakingTask task) {
        if (!task.started || task.getBlockState().isAir() || task.instantRemine || task.progress >= 1.0f) {
            return;
        }
    ...
    private void finishMining(BlockBreakingTask task) {
        if (!task.started || MC.player == null || MC.level == null) return;
        ...
        if (task.instantRemine && now - lastInstantBreak < instantDelay.getLong()) {
            return;
        }
        ...
        if (task.instantRemine && task.getBlockState().isAir() && now - lastServerBreak > 250L) {
            return;
        }
        ...
        if (simulate.get() && !task.getBlockState().isAir()) {
            MC.level.destroyBlock(task.getBlockPos(), false, MC.player, 512);
        }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.packet() instanceof ClientboundBlockUpdatePacket packet
                && packet.getPos().equals(currentTask.getBlockPos())) {
            currentTask.markBroken();
            lastServerBreak = System.currentTimeMillis();
        }
    }

    private void sendAction(ServerboundPlayerActionPacket.Action action, BlockBreakingTask task) {
    ...
    private void rotateServer(BlockPos pos) {
        ...
        Vec3 target = closestPoint(MC.player.getEyePosition(), new AABB(pos));
    ...
    private void switchTo(int slot, boolean silent) {
    ...
    private void restoreSilentSlot() {
        if (MC.player == null || silentPreviousSlot < 0) return;

        MC.player.connection.send(new ServerboundSetCarriedItemPacket(silentPreviousSlot));
        silentSlot = -1;
        silentPreviousSlot = -1;
    }

    private int getBestToolSlot(net.minecraft.world.level.block.state.BlockState state) {
        ...
        for (int slot = 0; slot < 9; slot++) {
            ...
            float toolSpeed = getToolSpeed(stack, state);
    ...
    private float getToolSpeed(ItemStack stack, net.minecraft.world.level.block.state.BlockState state) {
        if (!stack.isCorrectToolForDrops(state)) return 1.0f;

        int efficiency = getEfficiency(stack);
        return stack.getDestroySpeed(state) * (1.0f + efficiency * 0.2f);
    }
    ...
    private float calculateBlockDamage(net.minecraft.world.level.block.state.BlockState state, BlockPos pos) {
    ...
    private boolean canHarvest(net.minecraft.world.level.block.state.BlockState state) {
        if (!state.requiresCorrectToolForDrops()) return true;

        int slot = getBestToolSlot(state);
        if (slot < 0) return false;

        return MC.player.getInventory().getItem(slot).isCorrectToolForDrops(state);
    }

    private float getMiningSpeed(net.minecraft.world.level.block.state.BlockState state) {
        ItemStack stack;

        if (swap.get() == Swap.SILENT) {
            int slot = getBestToolSlot(state);
            stack = slot >= 0 ? MC.player.getInventory().getItem(slot) : MC.player.getMainHandItem();
        } else {
            stack = MC.player.getMainHandItem();
        }

        float value = stack.getDestroySpeed(state);

        if (value > 1.0f) {
            int efficiency = getEfficiency(stack);
            if (efficiency > 0) {
                value += efficiency * efficiency + 1;
            }
        }

        MobEffectInstance haste = MC.player.getEffect(MobEffects.HASTE);
        if (haste != null) {
            value *= 1.0f + (haste.getAmplifier() + 1) * 0.2f;
        }

        MobEffectInstance fatigue = MC.player.getEffect(MobEffects.MINING_FATIGUE);
        if (fatigue != null) {
            value *= switch (fatigue.getAmplifier()) {
                case 0 -> 0.3f;
                case 1 -> 0.09f;
                case 2 -> 0.0027f;
                default -> 0.00081f;
        ...
        boolean aquaAffinity = hasAquaAffinity(helmet);

        if (MC.player.isEyeInFluid(FluidTags.WATER) && !aquaAffinity) {
            value /= 5.0f;
        }
    ...
    private Vec3 closestPoint(Vec3 point, AABB box) {
        return new Vec3(
    ...
    private void renderProgress(Render3DEvent event, BlockBreakingTask task) {
        ...
        VoxelShape shape = task.instantRemine
                ? Shapes.block()
                : task.getStartState().getShape(MC.level, pos);
        ...
        float progress = task.previousProgress
                + (task.progress - task.previousProgress) * event.getDelta();
        float scale = Math.max(0.0f, Math.min(1.0f, progress / Math.max(task.targetSpeed, 0.0001f)));

        double dx = (bounds.maxX - bounds.minX) * 0.5;
        double dy = (bounds.maxY - bounds.minY) * 0.5;
        double dz = (bounds.maxZ - bounds.minZ) * 0.5;

        AABB renderBox = new AABB(center, center).inflate(dx * scale, dy * scale, dz * scale);
        int green = (int) (200.0f * scale);
        int red = 200 - green;
    ...
    public static final class BlockBreakingTask {
        ...
        public BlockBreakingTask(BlockPos pos, Direction facing, float speed) {
            this.blockPos = pos;
            this.facing = facing;
        ...
        public net.minecraft.world.level.block.state.BlockState getStartState() {
            ...
            if (!state.isAir() && state.getBlock() != startState.getBlock()) {
                startState = state;
            }
            return startState;
        }
