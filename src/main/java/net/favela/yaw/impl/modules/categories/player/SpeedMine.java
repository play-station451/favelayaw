...
public class SpeedMine extends Module {
    ...
    public final BooleanSetting instantRebreak = register(
            new BooleanSetting(
                    "InstantRebreak",
                    "Immediately starts the next block with no delay",
                    true
            )
    );
    public final BooleanSetting autoSwitch = register(
            new BooleanSetting(
                    "AutoSwitch",
                    "Silently switches to the best pickaxe while mining",
                    true
            )
    );
    public final ColorSetting boxColor = color(
            "Color",
            new Color(167, 123, 234, 120),
            true
    );
    ...
    public void onTick() {
        ...
        for (int i = active.size() - 1; i >= 0; i--) {
            ...
            long requiredAt70 = Math.max(
                    1L,
                    (long) (t.requiredMs * 0.70)
            );

            if (elapsed < requiredAt70) continue;
    ...
    private long breakTimeMs(BlockState state, BlockPos pos) {
        ...
        int ticks = Math.max(
                1,
                (int) Math.ceil((hardness * 30f) / toolSpeed)
        );
    ...
    private void beginPickaxeHold() {
        ...
        MC.player.connection.send(
                new ServerboundSetCarriedItemPacket(pickaxeSlot)
        );
    }
    ...
    private void endPickaxeHold() {
        ...
        MC.player.connection.send(
                new ServerboundSetCarriedItemPacket(originalSlot)
        );
    ...
    private int findBestPickaxe() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            float speed = stack.getDestroySpeed(
    ...
    private List<BlockPos> buildPattern(
            BlockPos primary,
            Direction face,
            int count
    ) {
